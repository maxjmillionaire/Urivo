...
/*
 * One-click unsubscribe from a campaign footer link (public — the recipient is
 * not signed in). The token is the only credential; the definer RPC sets
 * unsubscribed_at by token and is idempotent, so a second click is fine. The
 * page never reveals whether the token matched a real subscriber — it always
 * confirms — so the endpoint can't be used to test which addresses a store holds.
 */
...
export async function GET(request: NextRequest) {
  ...
  if (!UUID.test(token)) {
    return page("Link not recognised", "That unsubscribe link isn't valid. If you keep getting emails, reply to one and ask to be removed.");
  }
  ...
  try {
    ...
    await supabaseAdmin().rpc("unsubscribe_by_token", { p_token: token });
  } catch (err) {
    captureException(err, { requestId: newRequestId(), route: "unsubscribe" });
    return page("Something went wrong", "We couldn't process that just now. Please open the link again in a moment.");
  }

  return page("You're unsubscribed", "You won't receive any more emails from this store. You can re-subscribe anytime from the store itself.");
}
