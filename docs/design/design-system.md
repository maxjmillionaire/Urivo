...
| File | Use |
|---|---|
| `assets/brand/urivo-logo.png` | **Official brand master** (founder original, 1024×1024 app-icon badge) — app icon, favicon source, social avatars, marketing |

The founder's original artwork is the single source of truth — no redrawn or
approximated versions are permitted. Derived sizes (favicon 32/180/512, og-image)
are generated by downscaling this file, never by re-creation. Note: the master has
a white margin outside the rounded square; crop to the badge bounds when a
full-bleed icon is required. Minimum clear space = height of the arrowhead.
